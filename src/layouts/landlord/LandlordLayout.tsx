...
const landlordNavItems: Array<{ label: string; to: string; icon: FC<{ className?: string }>; planned?: boolean }> = [
  { label: 'Dashboard', to: ROUTES.LANDLORD_DASHBOARD, icon: LayoutDashboard },
  { label: 'My Properties', to: ROUTES.LANDLORD_PROPERTIES, icon: Building2 },
  { label: 'Tenants', to: ROUTES.LANDLORD_TENANTS, icon: Users, planned: true },
  { label: 'Leases', to: ROUTES.LANDLORD_LEASES, icon: FileText, planned: true },
  { label: 'Payments', to: ROUTES.LANDLORD_PAYMENTS, icon: CreditCard, planned: true },
  { label: 'Maintenance', to: ROUTES.LANDLORD_MAINTENANCE, icon: Wrench, planned: true },
  { label: 'Reports', to: ROUTES.LANDLORD, icon: BarChart3, planned: true },
];
...
function getInitials(name: string, fallback: string): string {
  const source = name.trim() || fallback.trim() || 'Landlord';
...
function BrandLogo() {
  return (
    <div className="flex items-center gap-3">
      <div className="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-amber-500 to-orange-500 shadow-lg shadow-amber-500/25">
      ...
      <div className="min-w-0">
        ...
        <p className="text-[11px] uppercase tracking-[0.22em] text-slate-400">Landlord portal</p>
      </div>
    </div>
  );
}
  ...
  return (
    <div className="flex h-full flex-col">
      <div className="border-b border-slate-800/50 px-5 py-5">
        <BrandLogo />
        <div className="mt-3">
          <RoleBadge role={roles[0] ?? 'Landlord'} />
        </div>
      </div>
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        <ul className="space-y-1">
          {landlordNavItems.map(({ label, to, icon: Icon, planned }) => (
            <li key={to}>
              {planned ? (
...
function LandlordTopbar() {
  ...
  const [isDarkMode, setIsDarkMode] = useState(() => {
    ...
    const stored = window.localStorage.getItem('landlord-theme');
  ...
  useEffect(() => {
    ...
    window.localStorage.setItem('landlord-theme', isDarkMode ? 'dark' : 'light');
  }, [isDarkMode]);
    ...
    <div className="flex min-w-0 items-center justify-end gap-3">
      <label className="relative hidden min-w-0 flex-1 sm:block max-w-md">
        ...
        <input
          ...
          className="h-10 w-full rounded-full border border-slate-200 bg-slate-50 pl-10 pr-4 text-sm text-slate-700 outline-none transition placeholder:text-slate-400 focus:border-amber-500 focus:bg-white"
        />
      </label>
      ...
      <div className="relative">
        ...
        >
          <span className="flex h-8 w-8 items-center justify-center rounded-full bg-gradient-to-br from-amber-400 to-orange-500 text-xs font-bold text-white">
            {initials}
          </span>
...
export function LandlordLayout() {
  return <DashboardLayout sidebar={<LandlordSidebar />} topbar={<LandlordTopbar />} />;
}